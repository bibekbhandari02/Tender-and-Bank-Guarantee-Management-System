      ...
      <div className="flex-1 rounded-xl overflow-hidden bg-gray-100">
        {isImage(file.file_type) ? (
          <div className="w-full h-full flex items-center justify-center p-4">
            <img src={file.url} alt={file.file_name} className="max-w-full max-h-full object-contain rounded-lg" />
          </div>
        ) : (
          <iframe src={file.url} title={file.file_name} className="w-full h-full border-0" />
        )}
      </div>
    </div>
  </div>
        ...
        {files.map((file) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-400">{isPdf(file.file_type) ? 'PDF Document' : 'Image'}</p>
            </div>

            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button type="button" onClick={() => setPreview(file)}
                className="p-1.5 rounded-lg text-gray-400 hover:bg-blue-50 hover:text-blue-600 transition-colors" title="View">
                <EyeIcon className="w-4 h-4" />
              </button>
              <button type="button" onClick={() => downloadFile(file.url, file.file_name)}
                className="p-1.5 rounded-lg text-gray-400 hover:bg-primary-50 hover:text-primary-600 transition-colors" title="Download">
                <ArrowDownTrayIcon className="w-4 h-4" />
              </button>
              {onDelete && (
                <button type="button" onClick={() => onDelete(file._id)}
